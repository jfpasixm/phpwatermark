    ...
    if(isset($_POST['process'])){
        ...
        if(isset($_FILES['filejpg']) && $_FILES['filejpg']['error']==0){
			...
			if (!file_exists('results')) {
				...
				touch(".htaccess");//creates htaccess
				rename(".htaccess", "results/.htaccess");
				$filecreated = "results/.htaccess";
				$myfile = fopen($filecreated, "w") or die("Unable to open file!");//open htaccess file with 'Open a file for write only' permission
				$txt = 'Options -Indexes';

				fwrite($myfile, $txt);//modify file
